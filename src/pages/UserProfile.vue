  ...
  <q-page>
    <q-form @submit="submitForm" class="inputForm">
      <div class="inputs">
        ...
        <q-input type="number" outlined v-model="tezina" label="Vaša težina (u kg)" />
        <q-input type="number" outlined v-model="visina" label="Vaša visina (u cm)" />
      ...
      <q-select
       ...
       />
        ...
        <q-btn class="q-ma-m" type="submit" color="primary" @click="spremiProfil()" label="Spremi profil" v-if="!uredi"/>
        <q-btn class="q-ma-m" type="submit" color="primary" @click="urediProfil()" label="Ažuriraj profil" v-if="uredi"/>
...
import {db, onAuthStateChanged, auth, doc, setDoc, getDoc, onSnapshot} from '../boot/firebase.js';
...
export default {
  ...
  methods: {
    ...
    calculateBMI() {
      ...
      const multiplier = (this.atletskaSpremnost == "Početnik" ? 1.3 :
      (this.atletskaSpremnost == "Rekreacijski" ? 1.5 :
      (this.atletskaSpremnost == "Natjecateljski" ? 1.7 : null)));

      const BMR = Math.round(10*this.tezina + 6.25*this.visina
      - 5*this.godine + (this.selectedGender === 'Musko' ? 5 : -161));
  ...
  beforeRouteUpdate(to, from, next){
    const userStore = useUserStore()
     if(userStore.fetchUser() == undefined) {
      next(false);  // ovo će spriječiti učitavanje stranice
    } else {
      this.email = userStore.fetchUser().email;

      next(); // ovo će dozvoliti učitavanje stranice
    }
  },
  mounted() {
