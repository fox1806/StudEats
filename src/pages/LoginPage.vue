  ...
  <div class="q-pa-md q-gutter-md loginScreen">
    <q-card>
      <q-card-section>
        ...
        <p v-if="showPasswordError" class="error-message">Pogrešna lozinka. Pokušajte ponovo.</p>
        <p v-if="showRequests" class="error-message">Previše pokušaja prijave. Molimo pokušajte kasnije.</p>
        <div class="register" v-if="showRequests">
          Zaboravili ste lozinku?
           <span class="clickable-text" @click="zaboravljenaLozinka" @mouseover="hover = true" @mouseleave="hover = false">
            Zatražite novu
           </span>
        </div>
        ...
        <div class="register">
          Niste korisnik? <router-link to="/register">Registrirajte se</router-link>
        </div>
  ...
  export default {
    ...
    async zaboravljenaLozinka() {
      ...
      try {
        ...
        this.$q.notify({
          color: 'negative',
          message: 'Greška pri slanju zahtjeva za reset lozinke: ' + error.message,
          position: 'top',
          timeout: 2000,
        });
      }
