<template>
  ...
  <div class="q-ma-md">
    <Doughnut :data="dataChart" :options="chartOptions" :key="chartKey" class="doughnut"/>
  </div>
    ...
    <div class="flex flex-center q-mt-xl" style="flex-direction: column;">
        ...
        <q-btn class="q-mt-xl" round icon="add" type="button" color="secondary" @click="showDialog = true"/>
        Dodaj jelo
      </div>
    ...
    <q-td :props="props">
      <q-btn icon="delete" flat round dense @click="pripremiZaBrisanje(props)" style="color: rgb(38,166,154);"/>
    </q-td>
  </template>
  ...
  <div>
    ...
    <q-dialog v-model="showDialogDelete">
      <q-card>
        ...
        <q-card-actions>
          <q-btn label="Odustani" color="primary" flat @click="showDialogDelete = false" />
          <q-btn label="Izbriši" color="negative" flat @click="brisanjeElementa()" />
        </q-card-actions>
      </q-card>
    </q-dialog>
  </div>
...
import { db, collection, query, where, onSnapshot, doc, deleteDoc} from '../boot/firebase.js';
...
export default defineComponent({
  ...
  data() {
    return {
      ...
      jela: [],
      columns: [
        { name: 'delete', required: true, label: '', align: 'left', field: 'delete', format: (val, row) => '', sortable: false},
        { name: 'naziv', required: true, label: 'Naziv', align: 'left', field: 'naziv' },
        { name: 'kalorije', required: true, label: 'Kalorije', align: 'left', field: 'kalorije' },
        { name: 'proteini', sortable:true, required: true, label: 'Proteini', align: 'left', field: 'proteini' },
        { name: 'ugljikohidrati', required: true, label: 'Ugljikohidrati', align: 'left', field: 'ugljikohidrati' },
        { name: 'masti', required: true, label: 'Masti', align: 'left', field: 'masti' }
      ],
      loading: false,
      TDEE: null,
  ...
  methods: {
    ...
    async unosTablica(popisJela) {
      ...
      for (let jelo of popisJela) {
          try {
              const response = await fetch(`${baseUrl}?term=${encodeURIComponent(jelo[0])}`, {
                cors: 'no-cors'
              });
