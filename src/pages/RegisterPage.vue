  ...
  <div class="q-pa-md q-gutter-md registerScreen">
    <q-card>
      <q-card-section>
        ...
        <q-input v-model="password" label="Lozinka" :type="isPwd ? 'password' : 'text'" >
      ...
      <q-input v-model="confirmPassword" label="Potvrda lozinke" :type="isPwdConfirm ? 'password' : 'text'">
        <template v-slot:append>
          <q-icon
        ...
        <q-btn color="primary" label="Registriraj se" @click="handleRegister" />
    ...
    async handleRegister() {
      ...
      try {
        ...
        if(register == 'userExists'){
          this.$q.notify({
            color: 'negative',
            message: 'Greška pri registraciji, korisnik s tim emailom već postoji',
            icon: 'report_problem',
            position: 'top',
          });
        }
